...
// [[Rcpp::export]]
double log_Prob_MBSigivenLj(const int& K, NumericVector& MBSi, IntegerVector& Lj, 
      NumericVector& bs_tpr, NumericVector& bs_fpr){
      double log_probs = 0.0;
      for(int k=0; k<K; ++k){
            double logP2 = MBSi[k]*(Lj[k]*log(bs_tpr[k]) + (1-Lj[k])*log(bs_fpr[k])) + \
                        (1-MBSi[k])*(Lj[k]*log(1-bs_tpr[k]) + (1-Lj[k])*log(1-bs_fpr[k]));
...
// [[Rcpp::export]]
NumericMatrix log_ProbMat_MBSgivenL(const int& K, IntegerMatrix& Lall_matrix,  
                  NumericMatrix& MBS, NumericVector& bs_tpr, NumericVector& bs_fpr){
      int J = Lall_matrix.nrow();
      int N = MBS.nrow();
      ...
      for(int i=0; i<N; ++i){
            ...
            for(int j=0; j<J; ++j){
                  IntegerVector lj = Lall_matrix(j,_);
                  condition_probs(i,j) = log_Prob_MBSigivenLj(K, mbsi, lj, \
                                                      bs_tpr=bs_tpr, bs_fpr=bs_fpr);
            }
      }
      return condition_probs;
}
...
double log_Prior_BetaTheta2_exch(NumericVector& Beta, double& Theta2, 
                              const double& var_beta, const double& var_theta2, const double& mean_theta2)
{
      double result = 0.0;
      for (int k=0; k<Beta.size(); ++k){
      ...
      result -= pow(Theta2 - mean_theta2, 2)/(2*var_theta2);
      return result;
}
/* --------------------------------------- EM components --------------------------------------------*/
...
// [[Rcpp::export]]
NumericMatrix EM_UpdateRates_fixSS(int& K, int& J1, int& N_case, int& N_ctrl, IntegerMatrix& MSS, IntegerMatrix& MBS_case, 
                        IntegerMatrix & MBS_ctrl, NumericMatrix& weights, IntegerVector& X_index, IntegerMatrix& Lmat_withZero,
                        NumericVector& aa, NumericVector& bb, NumericVector& cc, NumericVector& dd, NumericVector& ee, 
                        NumericVector& ff){
      //X_index starts from 0
...
class Qfunction_exch
{
public:
      typedef ::column_vector column_vector;
      Qfunction_exch(const NumericMatrix& Weights, const IntegerVector& X_index, const NumericMatrix& X_unique,
                  const IntegerMatrix& LUmat, const int& K, const int& J1, const int& D,
                  const double& prior_var_beta, const double& prior_var_theta2, const double& prior_mean_theta2)
      {
              this->weights = Weights;
      ...
      {                  
            ...
            NumericVector Theta2(this->Theta2_size);
            for (int k=0; k<this->Theta2_size; ++k){
                  Theta2(k) = Beta_Theta2(this->Beta_size); // using constant theta2
            }
            NumericMatrix qUnique(this->weights.nrow(), this->J1+1);
            ...
            return Qfunc_1 + log_Prior_BetaTheta2_exch(Beta, Theta2(0), vbeta, vtheta2, mtheta2);    
      }
...
// [[Rcpp::export]]
NumericMatrix EM_GetWeights_BSonly(int& K, IntegerMatrix& Lmat_withZero, NumericMatrix& MBS,
                              NumericVector& bs_tpr, NumericVector& bs_fpr,
                              IntegerVector& X_index, NumericMatrix& X_unique, IntegerMatrix& LUmat,
                              NumericVector& Beta, NumericVector& Theta2)
{
      ...
      log_ProbMat_a = log_ProbMat_MBSgivenL(K, Lmat_withZero, MBS, bs_tpr, bs_fpr);
...
// [[Rcpp::export]]
column_vector EM_UpdateBetaTheta2_exch(NumericMatrix& Weights, IntegerVector& X_index, NumericMatrix& X_unique,
                  IntegerMatrix& LUmat, int& K, int& J1, int& D,
                  double& prior_var_beta, double& prior_var_theta2, double& prior_mean_theta2,
                  NumericVector& initial_value)
{
      ...
      find_max_using_approximate_derivatives(bfgs_search_strategy(), objective_delta_stop_strategy(1e-8),
            Qfunction_exch(Weights, X_index, X_unique, LUmat, K, J1, D,
                      prior_var_beta, prior_var_theta2, prior_mean_theta2), 
            starting_point, 0.0, 1e-9);
      return starting_point;
}
...
class log_full_distn_exch
{
public:
      typedef ::column_vector column_vector;
      log_full_distn_exch( const IntegerVector& X_index, const NumericMatrix& X_unique,
                  const IntegerMatrix& LUmat, const int& K, const int& J1, const int& D, IntegerMatrix& L_withZero_mat,
                  NumericMatrix& MSS, NumericMatrix& MBS, NumericMatrix& MBS_ctrl,
                  const double& prior_var_beta, const double& prior_var_theta2, const double& prior_mean_theta2, 
                  const NumericVector& aa, const NumericVector& bb, const NumericVector& cc, const NumericVector& dd, 
      ...
      {                  
            ...
            NumericVector Theta2(this->Theta2_size);
            for (int k=0; k<this->Theta2_size; ++k){
                  Theta2(k) = All_Par(this->Beta_size + this->K*3 ); // constant theta2
            }
            ...
            return log_case + log_Prob_ctrl(this->K, this->mbs_ctrl.nrow(), this->mbs_ctrl, bs_fpr)+ 
                    rates_logprior + log_Prior_BetaTheta2_exch(Beta, Theta2(0), vbeta, vtheta2, mtheta2);    
      }
...
// TODO:
class log_full_distn_exch_BSonly
{
public:
      typedef ::column_vector column_vector;
      log_full_distn_exch_BSonly( const IntegerVector& X_index, const NumericMatrix& X_unique,
                  const IntegerMatrix& LUmat, const int& K, const int& J1, const int& D, IntegerMatrix& L_withZero_mat,
                  NumericMatrix& MSS, NumericMatrix& MBS, NumericMatrix& MBS_ctrl,
                  const double& prior_var_beta, const double& prior_var_theta2, const double& prior_mean_theta2, 
                  const NumericVector& aa, const NumericVector& bb, const NumericVector& cc, const NumericVector& dd, 
...
// [[Rcpp::export]]
double Evaluate_log_distn_exch(NumericVector& all_par, const IntegerVector& X_index, const NumericMatrix& X_unique,
                  const IntegerMatrix& LUmat, const int& K, const int& J1, const int& D, IntegerMatrix& L_withZero_mat,
                  NumericMatrix& MSS, NumericMatrix& MBS, NumericMatrix& MBS_ctrl,
                  const double& prior_var_beta, const double& prior_var_theta2, const double& prior_mean_theta2, 
                  const NumericVector& aa, const NumericVector& bb, const NumericVector& cc, const NumericVector& dd, 
                  const NumericVector& ee, const NumericVector& ff)
{     
      log_full_distn_exch logD( X_index, X_unique, LUmat, K, J1, D, L_withZero_mat, 
                   MSS, MBS, MBS_ctrl, prior_var_beta, prior_var_theta2, prior_mean_theta2, 
                   aa, bb, cc, dd, ee, ff);
